...
echo "building and pushing image nodejs-webapp"
pushd source
docker buildx build --platform ${PLATFORMS} -f Dockerfile --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/nodejs-webapp .
popd
